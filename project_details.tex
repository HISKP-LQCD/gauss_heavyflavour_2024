...
the total inclusive decay rate for the process $B_s \to X\ell\bar\nu$.
The simulations
...
We can extract the mass of the $B$ and $B_s$ pseudo-scalar meson ($M_{B}$ and $M_{B_s}$) meson, 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Sub-project 2: total inclusive decay rate $B_s \to X\ell\bar\nu$}

By using the optical theorem, the semileptonic decay of a $B$ and $B_s$ mesons to charmed
final states $X_c$ with a pair of massless leptons $B(B_s) \to X_c\ell\bar\nu$ can be written as
\begin{equation}
...
\begin{equation}\label{eq:Gamma_fg}
  \Gamma_{bc}=\int \frac{d^3p_\nu}{(2\pi)^32E_\nu}\frac{d^3p_\ell}{(2\pi)^32E_\ell}
  L_{\mu\nu}(p_\ell, p_\nu) H^{\mu\nu}_{bc}(p,p-p_\ell-p_\nu)\,,
\end{equation}
...
\begin{equation}
  H^{\mu\nu}_{bc}(p,p_X)=\frac{1}{2m_{D_s}}\langle D_s| J^\mu_{cb}(0)(2\pi)^4
  \delta^4(\mathbb{P}-p_x) J^{\nu\dagger}_{cb} (0)| D_s\rangle\,.
\end{equation}
$\mathbb{P}$ is the QCD four momentum operator and $J_{cb}$ are the
relevant Minkowski weak currents $J_{cb}^\mu(x)=i\bar
  c(x)\gamma^\mu(1-\gamma_5)b(x)$.
...
From now on we will drop the indices $bc$ 
...
using the same set of heavy quark mass as in the sub-project 1 of section~\ref{sec:mb}
